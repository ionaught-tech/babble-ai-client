...
interface UseFetchOptions {
  ...
  notLoad?: boolean;
  token?: string;
  paginationMergeFunction?: (r: any, p: any) => any;
}
...
const useFetch = <D, E = void>(
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const response = await getCall(url, option?.token, apiUrl);
      if (!signalRef.current) return;
      if (response.status) {
        setError(null);
        setData((prev) => {
          if (option?.paginationMergeFunction) {
            return option.paginationMergeFunction(response, prev);
          }
